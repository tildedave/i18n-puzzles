...
def answer(lines: List[str]):
    ...
    english_sorted = sorted(
        lines,
        key=cmp_to_key(
            partial(compare_lines, english_word_comparator, english_accent_stripper)
        ),
    )
    swedish_sorted = sorted(
        lines,
        key=cmp_to_key(
            partial(compare_lines, swedish_word_comparator, swedish_accent_stripper)
        ),
    )
    dutch_sorted = sorted(
        lines,
        key=cmp_to_key(
            partial(compare_lines, english_word_comparator, dutch_accent_stripper)
        ),
    )
    print("-----")
